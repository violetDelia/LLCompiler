...
#define DENSE_ATTR(Type)        \
  mlir::DenseElementsAttr::get( \
      type, llvm::ArrayRef<Type>((Type*)data, (Type*)(data + data_bytes)))
...
mlir::DenseElementsAttr loadNpyFile(mlir::ShapedType type,
  ...
  // auto type = npyTypeInfoToType(type_info.c_str(), build);
  // auto tensor = mlir::RankedTensorType::get(shape, type);
...
struct LoadWeightOp : public LLCOpRewritePattern<WeightOp> {
  ...
  void rewrite(WeightOp op, LLHPatternRewriter& rewriter) const final {
    ...
    rewriter.replaceOpWithNewOp<llh::ConstantOp>(op, value);
  }
};
//===----------------------------------------------------------------------===//
// pattern population
